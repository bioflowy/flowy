...
use crate::runtime::error::{RuntimeResult, IntoRuntimeError};
use std::fs;
...
pub fn write_file_atomic<P: AsRef<Path>, C: AsRef<[u8]>>(path: P, contents: C) -> RuntimeResult<()> {
    let path = path.as_ref();
    ...
    fs::write(&temp_path, contents)
        .runtime_context_with_path("Failed to write temporary file", &temp_path.display().to_string())?;
...
pub fn path_is_within<P: AsRef<Path>, Q: AsRef<Path>>(path: P, base: Q) -> RuntimeResult<bool> {
    ...
    let path = path_ref.canonicalize()
        .runtime_context_with_path("Failed to canonicalize path", &path_ref.display().to_string())?;
    let base = base_ref.canonicalize()
        .runtime_context_with_path("Failed to canonicalize base path", &base_ref.display().to_string())?;
...
impl WorkflowDirectory {
    ...
    pub fn stage_input<P: AsRef<Path>>(&self, source: P, name: &str, copy: bool) -> RuntimeResult<PathBuf> {
...
mod tests {
    ...
    fn test_workflow_directory() {
        ...
        let staged = workflow_dir.stage_input(&source_file, "input.txt", true).unwrap();
    ...
    fn test_temp_dir_creation() {
        ...
        assert!(temp_dir.file_name().unwrap().to_str().unwrap().starts_with("test_prefix"));
