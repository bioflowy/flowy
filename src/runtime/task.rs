...
impl TaskEngine {
    ...
    pub fn execute_task(
        ...
        match result {
            Ok(task_result) => {
                if options.verbose {
                    println!("Task {} completed successfully in {:?}", context.task.name, duration);
                }
                Ok(task_result)
            }
            Err(error) => {
                if options.verbose {
                    println!("Task {} failed after {:?}: {}", context.task.name, duration, error);
                }
                Err(error)
            }
        }
    ...
    pub fn validate_task(&self, task: &Task, inputs: &Bindings<Value>) -> RuntimeResult<()> {
                ...
                if input_decl.expr.is_none() { // Required input
                    if !inputs.has_binding(&input_decl.name) {
                        return Err(RuntimeError::WorkflowValidationError {
                            message: format!("Missing required input for task {}: {}", task.name, input_decl.name),
        ...
        for output_decl in &task.outputs {
            if output_decl.expr.is_none() {
                return Err(RuntimeError::WorkflowValidationError {
                    message: format!("Output declaration missing expression in task {}: {}", 
    ...
    pub fn create_task_directory(&self, task_name: &str, run_id: &str) -> RuntimeResult<PathBuf> {
        let task_dir = self.workflow_dir.work.join(format!("{}_{}", task_name, run_id));
...
pub mod utils {
    ...
    pub fn array_to_paths(value: &Value) -> Option<Vec<PathBuf>> {
        match value {
            Value::Array { values: arr, .. } => {
                let paths: Option<Vec<PathBuf>> = arr.iter()
                    .map(value_to_path)
                    .collect();
                paths
            }
    ...
    pub fn create_temp_file(content: &str, suffix: &str) -> RuntimeResult<PathBuf> {
        ...
        let filename = format!("wdl_temp_{}{}", 
            std::process::id(), 
            if suffix.starts_with('.') { suffix } else { &format!(".{}", suffix) }
        );
