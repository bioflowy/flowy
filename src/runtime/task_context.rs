...
use crate::runtime::fs_utils::{WorkflowDirectory, create_dir_all, write_file_atomic, read_file_to_string};
use crate::tree::Task;
...
impl TaskContext {
    ...
    fn validate_inputs(&self) -> RuntimeResult<()> {
        if let Some(inputs) = &self.task.inputs {
            for input_decl in inputs {
                if input_decl.expr.is_none() { // Required input (no default)
    ...
    fn prepare_environment(&self) -> RuntimeResult<()> {
        ...
        for binding in self.inputs.iter() {
            ...
            if let Value::File { value: file_path, .. } = value {
    ...
    fn generate_command(&self) -> RuntimeResult<String> {
        ...
        eval_env = eval_env.bind("task".to_string(), Value::String { value: self.task.name.clone(), wdl_type: Type::string(false) }, None);
        eval_env = eval_env.bind("sep".to_string(), Value::String { value: " ".to_string(), wdl_type: Type::string(false) }, None);
    ...
    fn execute_command(&self, command_str: &str) -> RuntimeResult<(ExitStatus, String, String)> {
        ...
        let script_content = format!("#!/bin/bash\nset -euo pipefail\ncd \"{}\"\n{}\n", 
            self.task_dir.display(), command_str);
        ...
        let child = cmd.spawn()
            .map_err(|e| RuntimeError::filesystem_error(
    ...
    fn wait_with_timeout(&self, child: std::process::Child, timeout: Duration) -> RuntimeResult<(ExitStatus, String, String)> {
        use std::thread;
        use std::sync::mpsc;
    ...
    fn collect_outputs(&self) -> RuntimeResult<Bindings<Value>> {
        ...
        for output_decl in &self.task.outputs {
            if let Some(output_expr) = &output_decl.expr {
                ...
                return Err(RuntimeError::WorkflowValidationError {
                    message: format!("Output declaration missing expression: {}", output_decl.name),
    ...
    fn value_matches_type(&self, value: &Value, expected_type: &Type) -> bool {
        ...
        match (value, expected_type) {
            ...
            (Value::Map { pairs: map, .. }, Type::Map { key_type, value_type, .. }) => {
                map.iter().all(|(k, v) | {
                    self.value_matches_type(k, key_type) && self.value_matches_type(v, value_type)
                })
            }
            (Value::Pair { left, right, .. }, Type::Pair { left_type, right_type, .. }) => {
