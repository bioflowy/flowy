...
fn parse_and_eval(expr_str: &str, env: &Bindings<Value>, stdlib: &StdLib) -> Result<Value, WdlError> {
    let doc = parser::parse_document(&format!("version 1.0\ntask test {{ command {{}} output {{ Int x = {} }}}}", expr_str), "1.0")?;
...
mod expression_render_tests {
    ...
    fn test_expr_render_literals() {
        ...
        assert_eq!(format!("{}", string_expr), r#""Some text with a ~{placeholder}""#);
    }
    ...
    fn test_expr_render_collections() {
        ...
        // Map literal
        let map_expr = Expression::map(pos.clone(), vec![
            (
    ...
    fn test_expr_render_comparisons() {
        ...
        let eq_expr = Expression::binary_op(pos.clone(), BinaryOperator::Equal, one.clone(), two.clone());
        assert_eq!(format!("{}", eq_expr), "1 == 2");
        
        let ne_expr = Expression::binary_op(pos.clone(), BinaryOperator::NotEqual, one.clone(), two.clone());
        assert_eq!(format!("{}", ne_expr), "1 != 2");
        
        let ge_expr = Expression::binary_op(pos.clone(), BinaryOperator::GreaterEqual, one.clone(), two.clone());
        assert_eq!(format!("{}", ge_expr), "1 >= 2");
        
        let le_expr = Expression::binary_op(pos.clone(), BinaryOperator::LessEqual, one.clone(), two.clone());
        assert_eq!(format!("{}", le_expr), "1 <= 2");
        
        let gt_expr = Expression::binary_op(pos.clone(), BinaryOperator::Greater, one.clone(), two.clone());
        assert_eq!(format!("{}", gt_expr), "1 > 2");
        
        let lt_expr = Expression::binary_op(pos.clone(), BinaryOperator::Less, one.clone(), two.clone());
    ...
    fn test_expr_render_arithmetic() {
        ...
        let add_expr = Expression::binary_op(pos.clone(), BinaryOperator::Add, one.clone(), one.clone());
        assert_eq!(format!("{}", add_expr), "1 + 1");
        
        let sub_expr = Expression::binary_op(pos.clone(), BinaryOperator::Subtract, one.clone(), one.clone());
        assert_eq!(format!("{}", sub_expr), "1 - 1");
        
        let div_expr = Expression::binary_op(pos.clone(), BinaryOperator::Divide, one.clone(), one.clone());
        assert_eq!(format!("{}", div_expr), "1 / 1");
        
        let mul_expr = Expression::binary_op(pos.clone(), BinaryOperator::Multiply, one.clone(), one.clone());
        assert_eq!(format!("{}", mul_expr), "1 * 1");
        
        let rem_expr = Expression::binary_op(pos.clone(), BinaryOperator::Modulo, one.clone(), one.clone());
    ...
    fn test_expr_render_access() {
        ...
        let array_access = Expression::get(
            pos.clone(),
            Expression::array(pos.clone(), vec![
                Expression::int(pos.clone(), 1),
                Expression::int(pos.clone(), 2),
            ]),
            Expression::int(pos.clone(), 1),
        ...
        let map_access = Expression::get(
            pos.clone(),
            Expression::map(pos.clone(), vec![
                (Expression::int(pos.clone(), 1), Expression::int(pos.clone(), 2)),
            ]),
            Expression::int(pos.clone(), 1),
        );
...
mod compound_equality_tests {
    ...
    fn test_array_equality() {
        ...
        let arr2 = Expression::array(pos.clone(), vec![
            Expression::int(pos.clone(), 1),
            Expression::int(pos.clone(), 2),
            Expression::int(pos.clone(), 3),
        ]);
        
        let arr3 = Expression::array(pos.clone(), vec![
            Expression::int(pos.clone(), 2),
            Expression::int(pos.clone(), 1),
            Expression::int(pos.clone(), 3),
        ]);
    ...
    fn test_map_equality() {
        ...
        let map2 = Expression::map(pos.clone(), vec![
            (
                Expression::string(pos.clone(), vec![StringPart::Text("a".to_string())]),
                Expression::int(pos.clone(), 1),
            ),
...
mod stdlib_function_tests {
    ...
    fn test_basename_function() {
        ...
        if let Some(basename_fn) = stdlib.get_function("basename") {
            // Test without suffix
            let result = basename_fn.eval(&[Value::string("/path/to/file.txt".to_string())]).unwrap();
            assert_eq!(result, Value::string("file.txt".to_string()));
            
            // Test with suffix - need to pass two arguments
            let result = basename_fn.eval(&[
