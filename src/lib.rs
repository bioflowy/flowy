...
pub use tree::{
    ASTNode, Call, Conditional, Declaration, Document, ImportDoc, Scatter, Task, Workflow,
    WorkflowNode,
};
pub use types::Type;
...
pub fn load(
    ...
    let path_vec = path
        ...
        .unwrap_or_default();
    futures::executor::block_on(load_async(
        uri.to_string(),
        path_vec,
        check_quant,
        import_max_depth,
...
fn _load_async(
    ...
    Box::pin(async move {
        ...
        // Parse document
        let mut doc =
            parser::parse_document_with_filename(&read_result.source_text, "1.2", &abspath)?;
...
pub async fn resolve_file_import(
    ...
    let candidate_path = if Path::new(uri).is_absolute() {
        ...
        match found_path {
            ...
            None => {
                return Err(WdlError::import_error(
                    SourcePosition::new(uri.to_string(), uri.to_string(), 1, 1, 1, 1),
                    uri.to_string(),
...
pub async fn read_source_default(
    ...
    let source_text = fs::read_to_string(&abspath).map_err(|e| {
        WdlError::import_error(
            SourcePosition::new(uri.to_string(), abspath.clone(), 1, 1, 1, 1),
            uri.to_string(),
