...
impl ValueBase for Value {
    ...
    fn coerce(&self, desired_type: &Type) -> Result<Value, WdlError> {
        ...
        match (self, desired_type) {
            ...
            // Array coercions
            (Value::Array { values, .. }, Type::Array { item_type, nonempty, .. }) => {
                ...
                let coerced_values: Result<Vec<_>, _> = values
                    .iter()
                    .map(|v| v.coerce(item_type))
                    .collect();
            ...
            // Map coercions
            (Value::Map { pairs, .. }, Type::Map { key_type, value_type, .. }) => {
                let coerced_pairs: Result<Vec<_>, _> = pairs
                    .iter()
                    .map(|(k, v)| {
                        Ok((k.coerce(key_type)?, v.coerce(value_type)?))
                    })
                    .collect();
