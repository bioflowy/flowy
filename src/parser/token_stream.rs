...
impl TokenStream {
        ...
        loop {
            ...
            match tokenizer(input) {
                Ok((remaining, token)) => {
                    ...
                    if matches!(token.token, Token::Whitespace(_) | Token::Newline | Token::Comment(_)) {
    ...
    pub fn expect(&mut self, expected: Token) -> Result<LocatedToken, WdlError> {
        ...
        match current_token {
            ...
            Some(token) => {
                let pos = current_pos.unwrap_or_else(|| SourcePosition::new("".to_string(), "".to_string(), 1, 1, 1, 1));
                Err(WdlError::syntax_error(
                    pos,
