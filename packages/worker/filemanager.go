...
type FileManager interface {
	GetLocalPath(fileurl string, localPath *string) (string, error)
...
func (f *LocalFileManager) GetLocalPath(fileurl string, localPath *string) (string, error) {
	if localPath == nil {
		// do nothing when shared file system is available
		return fileurl, nil
	} else {
		err := symlink(fileurl, *localPath, false)
		if err != nil {
...
func (f *S3FileManafer) GetLocalPath(fileurl string, localPath *string) (string, error) {
	// if file exists on s3 bucket, download that to tmpfile and return tmpfile path
	return downloadS3File(f.session, fileurl, localPath)
}
func (f *S3FileManafer) GetType() string {
	// do nothing when shared file system is available
	return "s3"
}
func (f *S3FileManafer) GetFileUrl(localPath string, fileurl *string) (string, error) {
...
func uploadFile(uploader *s3manager.Uploader, fileManager *S3FileManafer, filePath string, s3url *string) (string, error) {
...
func uploadDirectory(uploader *s3manager.Uploader, fileManager *S3FileManafer, directoryPath string, s3url *string) (string, error) {
	u, err := url.Parse(fileManager.RootUrl)
	if err != nil {
		return "", err
	}
